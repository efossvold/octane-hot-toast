...
type State = Record<string, ToasterState>
...
const listeners: [toasterId: string, reducer: (state: ToasterState) => void][] = []
...
export const dispatch = (action: Action, toasterId = DEFAULT_TOASTER_ID) => {
  memoryState[toasterId] = reducer(memoryState[toasterId] ?? defaultToasterState, action)
...
export const useStore = (
  ...
  const [state, setState] = useState<ToasterState>(memoryState[toasterId] ?? defaultToasterState)
